/**
 * Onboarding Bulk Upload Component
...
export function OnboardingBulkUpload({ onUploadComplete }: BulkUploadProps) {
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const requiredHeaders = ['Employee Name', 'Email', 'Department', 'Start Date'];
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        const candidateData: any = {
          employeeName: getCellValue('Employee Name'),
          email: getCellValue('Email'),
          department: getCellValue('Department'),
          position: getCellValue('Position'),
          startDate: getCellDate('Start Date'),
          manager: getCellValue('Manager'),
          buddy: getCellValue('Buddy'),
        ...
        if (candidateData.employeeName && candidateData.email && candidateData.department && candidateData.startDate) {
          candidatesData.push(candidateData);
        } else {
          console.warn(`Skipping row ${rowNumber}: missing required data`, {
            hasName: !!candidateData.employeeName,
            hasEmail: !!candidateData.email,
            hasDepartment: !!candidateData.department,
            hasStartDate: !!candidateData.startDate,
          });
        }
      });
      ...
      if (candidatesData.length === 0) {
        ...
        throw new Error('No valid candidate data found in Excel file. Please check that your Excel file has data rows with Employee Name, Email, Department, and Start Date filled in. Open browser console (F12) for detailed logs.');
      }
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-1fe2c468/onboarding/bulk-upload`,
        {
          method: 'POST',
          headers: {
  ...
  const downloadTemplate = () => {
    ...
    worksheet.columns = [
      ...
      { header: 'Buddy', key: 'buddy', width: 25 },
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      a.href = url;
      a.download = 'onboarding_upload_template.xlsx';
      a.click();
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload multiple onboarding candidates at once using an Excel file
          </CardDescription>
        </CardHeader>
            ...
            <AlertDescription>
              <div className="space-y-2">
                ...
                <ol className="list-decimal list-inside space-y-1 text-sm">
                  ...
                  <li>Required fields: Employee Name, Email, Department, Start Date</li>
                  <li>Optional fields: Position, Manager, Buddy, Notes</li>
