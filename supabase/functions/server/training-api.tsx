...
app.get('/stats', async (c) => {
  try {
    const supabase = getSupabase();
    const { data: enrollments } = await supabase.from('training_enrollments').select('status');
    const { data: courses } = await supabase.from('training_courses').select('status');

    const total = enrollments?.length || 0;
    const completed = enrollments?.filter((e: any) => e.status === 'Completed').length || 0;
    const inProgress = enrollments?.filter((e: any) => e.status === 'In Progress').length || 0;
    ...
    return c.json({
      ...
      data: {
        ...
        activeCourses: courses?.filter((c: any) => c.status === 'Active').length || 0,
