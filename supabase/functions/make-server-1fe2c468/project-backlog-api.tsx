...
app.get('/backlog', async (c) => {
  try {
    ...
    const unassigned = c.req.query('unassigned'); // 'true' = only items with sprint_id IS NULL
    const excludeStatus = c.req.query('excludeStatus'); // e.g. 'Done'
...
app.post('/backlog', async (c) => {
  try {
    ...
    // Get next ID from DB sequence (preferred); fall back to max-scan if RPC not deployed yet
    ...
    // Only include epic_id if provided — column may not exist in older schema
