...
app.get('/stats', async (c) => {
  try {
    ...
    const [candidatesResult, jobsResult] = await Promise.allSettled([
      supabase.from('recruitment_candidates').select('stage, status, applied_date, updated_at'),
    ...
    const offersExtended = candidates.filter((c: any) => c.stage === 'Offer Sent').length;
    ...
    // Hired this month based on updated_at date
    const hiredThisMonth = candidates.filter((c: any) => {
      if (c.stage !== 'Hired' || !c.updated_at) return false;
      const d = new Date(c.updated_at);
      return d.getMonth() === currentMonth && d.getFullYear() === currentYear;
    }).length;

    // Pipeline: candidates in active pipeline stages
    const pipelineCount = candidates.filter((c: any) =>
      ['Applied', 'Screening', 'Interview Scheduled', 'Interview Done', 'Offer Sent'].includes(c.stage)
    ).length;

    // Active jobs from the jobs table
    const activeJobs = jobs.filter((j: any) => j.status === 'Active' || j.status === 'Open').length;
